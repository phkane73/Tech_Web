  ...
  <main>
    <div class="container">
      <div class="row my-4" style="height: 40px">
        <RouterLink class="link" to="/" style="line-height: 40px"
          >Trang chủ</RouterLink
        >
        <i
          class="fa-solid fa-chevron-right"
          style="line-height: 40px; margin-left: 3px; margin-right: 3px"
        ></i>
        <RouterLink class="link" to="/products" style="line-height: 40px"
          >Sản phẩm</RouterLink
        >
        <i
          class="fa-solid fa-chevron-right"
        ...
        ></i>
        <span
          style="line-height: 40px; cursor: default; text-transform: capitalize"
          v-if="brand != 'all'"
          >{{ brand }}</span
        >
      </div>
      ...
      <div class="row mb-5">
        <ul class="laptop-brand d-flex justify-content-between w-100">
          <li class="laptop-brand__item">
            <div @click="getPage(0, 'macbook')" style="cursor: pointer">
              <img src="../assets/macbook-img.png" />
            </div>
          </li>
          <li class="laptop-brand__item">
            <div @click="getPage(0, 'asus')" style="cursor: pointer">
              <img src="../assets/asus-img.png" />
            </div>
          </li>
          <li class="laptop-brand__item">
            <div @click="getPage(0, 'dell')" style="cursor: pointer">
              <img src="../assets/dell-img.png" />
            </div>
          </li>
          <li class="laptop-brand__item">
            <div @click="getPage(0, 'hp')" style="cursor: pointer">
              <img src="../assets/hp-img.png" />
            </div>
          </li>
          <li class="laptop-brand__item">
            <div @click="getPage(0, 'acer')" style="cursor: pointer">
              <img src="../assets/acer-img.png" />
            </div>
          </li>
          <li class="laptop-brand__item">
            <div @click="getPage(0, 'lenovo')" style="cursor: pointer">
        ...
        <ul class="category-list d-flex">
          ...
          <RouterLink to="/products/earphones">
            <li class="category-item" style="margin-right: 40px">
              <img
                src="../assets/category_earphone.jpg"
                style="height: 118px; margin-top: 7px"
              />
              <span class="d-block text-center" style="margin-top: 5px"
                >Earphone</span
              >
            </li>
          </RouterLink>
        ...
        <ul class="pagination">
          <li class="page-item">
            <button
              ...
              @click="getPage(products.number - 1, brand)"
              v-if="products.number > 0"
            >
          ...
          <li
            class="page-item"
            v-for="page in products.totalPages - 2"
            v-bind:key="page"
          >
          ...
          <li class="page-item">
            <button
              class="page-link text-dark"
              @click="getPage(products.totalPages - 1, brand)"
            >
              Last
            </button>
          </li>
...
export default {
  ...
  methods: {
    getPage(page, brand, sort) {
      if (sort == undefined) {
        if (brand == undefined) {
          productServices.getPage(page, "all", "laptop", "").then((res) => {
            this.products = res.data;
          });
        } else {
          productServices.getPage(page, brand, "laptop", "").then((res) => {
            this.products = res.data;
            this.brand = brand;
          });
        }
        ...
        if (brand == undefined) {
          productServices.getPage(page, "all", "laptop", sort).then((res) => {
            this.products = res.data;
          });
        } else {
          productServices.getPage(page, brand, "laptop", sort).then((res) => {
            this.products = res.data;
            this.brand = brand;
          });
        }
