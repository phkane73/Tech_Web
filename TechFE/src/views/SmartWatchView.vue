        ...
        <ul class="laptop-brand d-flex justify-content-around w-100">
          <li class="laptop-brand__item">
            <div @click="getPage(0, 'Apple')" style="cursor: pointer">
              <img src="../assets/apple-watch.png" />
            </div>
          </li>
          <li class="laptop-brand__item">
...
export default {
  ...
  methods: {
    getPage(page, brand, sort) {
      if (sort == undefined) {
        if (brand == undefined) {
          productServices.getPage(page, "all", "smartwatch", "").then((res) => {
            this.products = res.data;
          });
        } else {
          productServices.getPage(page, brand, "smartwatch", "").then((res) => {
            this.products = res.data;
            this.brand = brand;
          });
        }
        ...
        if (brand == undefined) {
          productServices
            .getPage(page, "all", "smartwatch", sort)
            .then((res) => {
              this.products = res.data;
            });
        } else {
          productServices
            .getPage(page, brand, "smartwatch", sort)
            .then((res) => {
              this.products = res.data;
              this.brand = brand;
            });
