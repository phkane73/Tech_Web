  ...
  <main>
    <div class="container">
      ...
      <h1 class="category-name text-uppercase text-center my-5">
        Các Sản phẩm earphone
      </h1>
      <div class="row mb-5">
        <ul class="laptop-brand d-flex justify-content-around w-100">
          <li class="laptop-brand__item">
            <div @click="getPage(0, 'Apple')" style="cursor: pointer">
              <img src="../assets/apple-ep.jpg" />
            </div>
          </li>
          <li class="laptop-brand__item">
            <div @click="getPage(0, 'Samsung')" style="cursor: pointer">
              <img
                src="../assets/samsung-img.png"
                style="background-color: white"
              />
            </div>
          </li>
          <li class="laptop-brand__item">
            <div @click="getPage(0, 'SONY')" style="cursor: pointer">
              <img
                src="../assets/sony-ep.png"
                style="background-color: white"
              />
            </div>
          </li>
...
export default {
  ...
  methods: {
    getPage(page, brand, sort) {
      if (sort == undefined) {
        if (brand == undefined) {
          productServices.getPage(page, "all", "earphone", "").then((res) => {
            this.products = res.data;
          });
        } else {
          productServices.getPage(page, brand, "earphone", "").then((res) => {
            this.products = res.data;
            this.brand = brand;
          });
        }
        ...
        if (brand == undefined) {
          productServices.getPage(page, "all", "earphone", sort).then((res) => {
            this.products = res.data;
          });
        } else {
          productServices.getPage(page, brand, "earphone", sort).then((res) => {
            this.products = res.data;
            this.brand = brand;
          });
        }
